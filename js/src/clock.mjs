...
function changeClcok() {
  ...
  //시계 - 두자리 수를 맞출 것
  $clockDigital.textContent = `${nowHours >= 10 ? nowHours : "0" + nowHours}:${
  ...
  $clockPercent.textContent = `${
    nowHours >= startTime && nowHours < endTime
      ? Math.ceil(
          ((Math.abs(nowHours - startTime) + nowMinutes / 60) / percentUnit) *
            100
        )
      : nowHours >= endTime
      ? "+" +
      ...
      : "+" +
        Math.ceil(
          ((nowHours + (24 - endTime) + nowMinutes / 60) / percentUnit) * 100
        )
  }%`;
...
$layoutClock.onclick = (e) => {
  ...
  $layoutClock
    ...
    .forEach((clock) => clock.classList.toggle("clock-active"));
};
